...
#       등장한 횟수가 3보다 큰 수는 없기 때문에, NGF(1) = -1이다. A3의 경우에는 A7이 오른쪽에 있으면서 F(A3=2) < F(A7=1)
#       이기 때문에, NGF(3) = 1이다. NGF(4) = 2, NGF(5) = 2, NGF(6) = 1 이다.

# 입력) 첫째 줄에 수열 A의 크기 N (1 ≤ N ≤ 1,000,000)이 주어진다. 둘째에 수열 A의 원소 A1, A2, ..., AN
#       (1 ≤ Ai ≤ 1,000,000)이 주어진다.

# 출력) 총 N개의 수 NGF(1), NGF(2), ..., NGF(N)을 공백으로 구분해 출력한다.

# 문제 소스) 백준 - https://www.acmicpc.net/problem/17299
...
# 답 구하기
for i in range(N):
    while stack and temp[A[i]] > temp[A[stack[-1]]]:
        ans[stack[-1]] = A[i]
        stack.pop()        
    stack.append(i)
